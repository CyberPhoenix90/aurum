...
describe('DataSource extended contracts', () => {
    ...
    it('creates and cancels DOM event sources', () => {
        ...
        const target = {
            addEventListener: (_event: string, listener: (event: Event) => void) => listeners.add(listener),
            removeEventListener: (_event: string, listener: (event: Event) => void) => listeners.delete(listener)
        };
        const token = new CancellationToken();
        const source = DataSource.fromDomEvent<Event>(target, 'click', token);
        const click = new Event('click');
        listeners.forEach((listener) => listener(click));
        const first = source.value;
        token.cancel();
        listeners.forEach((listener) => listener(new Event('click')));
        assert.strictEqual(first, click);
