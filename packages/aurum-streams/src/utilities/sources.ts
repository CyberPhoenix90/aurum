...
import { Channel } from '../stream/channel.js';
...
export function getValueOf<T>(sourceOrPrimitive: Data<T> | Channel<any, T>): T;
export function getValueOf<T>(sourceOrPrimitive: Data<T> | DataArray<T> | Channel<any, T>): T | ReadonlyArray<T> {
    if (sourceOrPrimitive instanceof DataSource || sourceOrPrimitive instanceof Channel) {
