...
import { ArrayDataSource, DataSource } from '../sources/data_source.js';
import {
...
import { EventEmitter } from '../sources/event_emitter.js';

export interface StreamEventAcceptor<T> {
    onEvent(event: T): void;
}

export interface StreamEventProcessor<T, M> {
    onEvent(event: T): M;
}
 ...
 * Mutates an update
 */
export function dsProcess<T, M>(processor: StreamEventProcessor<T, M>): DataSourceMapOperator<T, M> {
    return {
        name: 'map',
        operationType: OperationType.MAP,
        operation: (v) => processor.onEvent(v)
    };
}
 ...
 */
export function dsPipe<T>(target: DataSource<T> | StreamEventAcceptor<T> | StreamEventProcessor<T, any>): DataSourceNoopOperator<T> {
    return {
        name: `pipe ${(target as any)?.name ?? 'Acceptor'}`,
        operationType: OperationType.NOOP,
        operation: (v) => {
 ...
 */
export function dsPipeUp<T>(target: DataSource<T> | StreamEventAcceptor<T> | StreamEventProcessor<T, any>): DataSourceNoopOperator<T> {
    return {
        name: `pipe ${(target as any)?.name ?? 'Acceptor'}`,
        operationType: OperationType.NOOP,
        operation: (v) => {
 ...
 */
export function dsLoadBalance<T>(targets: Array<DataSource<T> | StreamEventAcceptor<T> | StreamEventProcessor<T, any>>): DataSourceNoopOperator<T> {
    let i = 0;

    return {
        name: `loadBalance [${targets.map((v) => (v as any)?.name ?? 'Acceptor').join()}]`,
        operationType: OperationType.NOOP,
        operation: (v) => {
...
export function dsBroadcast<T>(...sources: Array<DataSource<T> | StreamEventAcceptor<T> | StreamEventProcessor<T, any>>): DataSourceNoopOperator<T> {
    return {
        name: `pipeAll [${sources.map((v) => (v as any)?.name ?? 'Acceptor').join()}]`,
        operationType: OperationType.NOOP,
        operation: (v) => {
