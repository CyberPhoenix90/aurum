...
export const textureMap: Map<
    | string
    | HTMLCanvasElement
    | HTMLImageElement
    | ResourceWrapper<HTMLImageElement, string>,
    BaseTexture
> = new Map();
export const pendingTextureMap: Map<
    string | ResourceWrapper<HTMLImageElement, string>,
...
export class RenderSpriteEntity extends NoRenderEntity {
    ...
    public static voidTexture: PixiTexture = new PixiTexture(
        new BaseTexture(c)
    );
    ...
    protected createTexture(model: SpriteGraphNode): PixiTexture {
        ...
        return this.wrapTexture(
            textureMap.get(model.resolvedModel.texture.value),
            model
        );
    }

    private handleTextureReady(
        texture: string | ResourceWrapper<HTMLImageElement, string>,
        img: HTMLImageElement,
        model: SpriteGraphNode
    ) {
    ...
    public bind(model: SpriteGraphNode) {
        ...
        model.resolvedModel.width.listenAndRepeat((v) => {
            if (v === "auto") {
                ...
                model.renderState.width.update(
                    this.displayObject.texture.baseTexture.realWidth
                );
            }
        });
        ...
        model.resolvedModel.height.listenAndRepeat((v) => {
            if (v === "auto") {
                ...
                model.renderState.height.update(
                    this.displayObject.texture.baseTexture.realHeight
                );
            }
        });
            ...
            (w, h, ddx, ddy, dox, doy, sx, sy) => {
                ...
                if (ddx === undefined) {
                    ...
                    if (typeof ddx === "number") {
                        ...
                        this.displayObject.texture.frame.width = ddx.toPixels(
                            ScreenHelper.PPI,
                            this.displayObject.texture.baseTexture.realWidth
                        );
                    }
                ...
                if (ddy === undefined) {
                    ...
                    if (typeof ddy === "number") {
                        ...
                        this.displayObject.texture.frame.height = ddy.toPixels(
                            ScreenHelper.PPI,
                            this.displayObject.texture.baseTexture.realHeight
                        );
                    }
