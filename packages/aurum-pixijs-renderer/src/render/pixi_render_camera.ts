...
export class RenderCameraEntity extends NoRenderEntity {
    ...
    constructor(model: CameraGraphNode, stageNode: HTMLElement) {
        ...
        view.width =
            model.resolvedModel.resolutionX.value ??
            model.renderState.width.value;
        view.height =
            model.resolvedModel.resolutionY.value ??
        ...
        this.renderer = autoDetectRenderer({
            ...
            backgroundColor: Color.fromString(
                model.renderState.backgroundColor.value || "#000000"
            ).toRGBNumber(),
        });
        ...
        resolutionX.aggregate(
            [resolutionY, model.renderState.width, model.renderState.height],
            (resolutionX, resolutionY, width, height) => {
                const effectivewidth = resolutionX ?? width;
                const effectiveheight = resolutionY ?? height;

                if (
                    this.renderer.view.width !== effectivewidth ||
                    this.renderer.view.height !== effectiveheight
                ) {
