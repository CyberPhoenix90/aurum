...
export class StreamLine {
    ...
    public getUId(): number {
        //@ts-ignore
        return StreamLine.idRoot++ + Math.random() * performance.now() * (performance?.memory?.usedJSHeapSize ?? 1);
    }
    ...
    public extendFunction<T extends Function>(this: any, original: T, modifications: { preFunction?: Function; postFunction?: Function }): T {
    ...
    public mapObjectValues<T>(object: Record<string, T>, cb: (value: T, key: string) => T): Record<string, T> {
    ...
    public objectFromKeyArray<T>(keyArray: string[], cb: (k: string) => T): Record<string, T> {
        const result: Record<string, T> = {};
